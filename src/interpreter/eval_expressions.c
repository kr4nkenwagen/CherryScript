...
#include "../predefined_functions/print.h"
#include "../predefined_functions/println.h"
#include "../program.h"
#include "../stack.h"
...
jl_object_t *eval_string_operation_expression(jl_syntax_t *syntax, vm_t *vm,
  ...
  jl_object_t *left_hand_side =
      eval_primary_expression(syntax->left, vm, program);
  jl_object_t *right_hand_side =
  ...
  if (syntax->token->type == COLON) {
    ...
    if (right_hand_side->type == STRING_OBJECT) {
      int position = jl_position_of_first_instance(
          left_hand_side, right_hand_side->data.v_string);
      if (position == -1) {
        return NULL;
      }
    ...
    if (right_hand_side->type == INT_OBJECT) {
      ...
      return jl_substring(left_hand_side, size - right_hand_side->data.v_int,
                          right_hand_side->data.v_int);
    }
    if (right_hand_side->type == STRING_OBJECT) {
      int position = jl_position_of_last_instance(
          left_hand_side, right_hand_side->data.v_string);
      if (position == -1) {
        return NULL;
      }
    ...
    if (left_hand_side->type == STRING_OBJECT ||
        right_hand_side->type == STRING_OBJECT) {
...
jl_object_t *eval_and_or(jl_syntax_t *syntax, vm_t *vm, jl_program_t *program) {
  ...
  jl_object_t *right_hand_side =
      eval_primary_expression(syntax->right, vm, program);
  if (left_hand_side->type != BOOL_OBJECT ||
      right_hand_side->type != BOOL_OBJECT) {
    return NULL;
  }
  if (syntax->token->type == AND) {
    return jl_new_bool(left_hand_side->data.v_bool &&
                       right_hand_side->data.v_bool);
