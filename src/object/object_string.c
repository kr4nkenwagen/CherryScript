...
void jl_strip_instances_from_string(jl_object_t *target,
                                    jl_object_t *instance) {
  if (target == NULL || instance == NULL || target->type != STRING_OBJECT ||
      instance->type != STRING_OBJECT) {
    return;
  }
  for (int i = 0; target->data.v_string[i] != '\0'; i++) {
    for (int x = 0; target->data.v_string[i + x] == instance->data.v_string[x];
         x++) {
      if (instance->data.v_string[x + 1] == '\0') {
        ...
        memcpy(tmp_string + i, target->data.v_string + i + x,
               target_size - i - x);
...
void jl_shorten_string(jl_object_t *target, jl_object_t *length) {
  if (target == NULL || length <= 0 || target->type != STRING_OBJECT ||
      length->type != INT_OBJECT) {
    return;
  }
...
void jl_multiply_string(jl_object_t *target, jl_object_t *multiplier) {
  if (target == NULL || multiplier == NULL || target->type != STRING_OBJECT ||
      multiplier->type != INT_OBJECT) {
    return;
  }
...
void jl_divide_string(jl_object_t *target, jl_object_t *divider) {
  if (target == NULL || divider == NULL || target->type != STRING_OBJECT ||
      divider->type != INT_OBJECT) {
    return;
  }
  size_t old_size = jl_string_length(target);
...
void jl_modulus_string(jl_object_t *target, jl_object_t *modulus) {
  if (target == NULL || modulus == NULL || target->type != STRING_OBJECT ||
      modulus->type != INT_OBJECT) {
    return;
  }
  size_t old_size = jl_string_length(target);
