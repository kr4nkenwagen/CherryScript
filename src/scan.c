...
jl_token_t *consume_reserved_word(jl_source_code_t *src) {
  ...
  case 'E':
    if (is_next_word_match(src, "else")) {
      if (tolower(jl_source_code_peek(src, 6)) == 'i' &&
          tolower(jl_source_code_peek(src, 7)) == 'f') {
        return jl_token_new(src, ELSE_IF,
                            strcat(consume_word(src), consume_word(src)));
      }
...
jl_token_list_t *scan(jl_source_code_t *src) {
  ...
  while (!src->is_at_end) {
    ...
    case '.':
      if (is_number(jl_source_code_peek(src, 1))) {
        jl_token_list_add(token_list,
                          jl_token_new(src, NUMBER, consume_number(src)));
    ...
    case '"':
      jl_token_list_add(token_list,
                        jl_token_new(src, STRING_WRAPPER, consume_string(src)));
      break;
    case '!':
      if (jl_source_code_peek(src, 1) == '=') {
    ...
    case '9':
      ...
      break;
    case '\n':
      if (tmp != NULL && (tmp->type == LEFT_PAREN || tmp->type == TERMINATOR ||
                          tmp->type == LEFT_BRACE)) {
        break;
      }
    case ';':
  ...
  if (token_list->count > 0 &&
      jl_token_list_peek(token_list, 0)->type != TERMINATOR) {
    jl_token_list_add(token_list, jl_token_new(src, TERMINATOR, ";"));
  }
